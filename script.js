...
// ===== AJOUTS POUR RÉCOMPENSE, MARCHÉ, PROFIL =====
...
// Bouton transférer récompenses
const transferBtn = document.getElementById("transferRewardBtn");
if(transferBtn){
transferBtn.addEventListener("click", () => showPopup("Disponible bientôt"));
}
...
wheelCooldown.innerText = "Disponible maintenant";
spinBtn.disabled = false;
}else{
const lastSpin = parseInt(localStorage.getItem("lastSpin"));
const timeLeft = 86400000 - (Date.now() - lastSpin);
const hours = Math.floor(timeLeft / 3600000);
const minutes = Math.floor((timeLeft % 3600000) / 60000);
wheelCooldown.innerText = `Prochain spin dans ${hours}h ${minutes}min`;
...
if(isSpinning ||!checkWheelCooldown()) return;
isSpinning = true;
spinBtn.disabled = true;
...
isSpinning = false;
showPopup("Gagné! Vérifie tes récompenses");
updateWheelCooldown();
setTimeout(() => {
...
updateWheelCooldown();
setInterval(updateWheelCooldown, 60000);
}

// Timers 24h
function startTimer(elementId, key){
...
startTimer("dailyTimer", "dailyTimerEnd");
startTimer("chestTimer", "chestTimerEnd");
...
showPopup("Changement d'avatar bientôt disponible");
});
}
